...
/*   Updated: 2016/11/22 20:38:02 by daugier          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	WALL[3] = mlx_xpm_file_to_image(MLX, "Texture/feuille.xpm", &TEXT_WIDTH[3],
			&TEXT_HEIGHT[3]);
	WALL_DATA[3] = mlx_get_data_addr(WALL[3], &TEXT_BPP[3], &TEXT_SIZE_LINE[3],
			&ENDIAN);
	WALL[8] = mlx_xpm_file_to_image(MLX, "Texture/2.xpm", &TEXT_WIDTH[8],
			&TEXT_HEIGHT[8]);
	...
	WALL_DATA[5] = mlx_get_data_addr(WALL[5], &TEXT_BPP[5], &TEXT_SIZE_LINE[5],
			&ENDIAN);
	WALL[6] = mlx_xpm_file_to_image(MLX, "Texture/4.xpm", &TEXT_WIDTH[6],
			&TEXT_HEIGHT[6]);
	WALL_DATA[6] = mlx_get_data_addr(WALL[6], &TEXT_BPP[6], &TEXT_SIZE_LINE[6],
			&ENDIAN);
	WALL[4] = mlx_xpm_file_to_image(MLX, "Texture/telepo.xpm", &TEXT_WIDTH[4],
			&TEXT_HEIGHT[4]);
	WALL_DATA[4] = mlx_get_data_addr(WALL[4], &TEXT_BPP[4], &TEXT_SIZE_LINE[4],
			&ENDIAN);
	WALL[9] = mlx_xpm_file_to_image(MLX, "Texture/5.xpm", &TEXT_WIDTH[9],
			&TEXT_HEIGHT[9]);
	WALL_DATA[9] = mlx_get_data_addr(WALL[9], &TEXT_BPP[9], &TEXT_SIZE_LINE[9],
			&ENDIAN);
}

static void		init_floor_texture(t_struct *data)
{
	WALL[0] = mlx_xpm_file_to_image(MLX, "Texture/6.xpm", &TEXT_WIDTH[0],
			&TEXT_HEIGHT[0]);
	WALL_DATA[0] = mlx_get_data_addr(WALL[0], &TEXT_BPP[0], &TEXT_SIZE_LINE[0],
			&ENDIAN);
	WALL[1] = mlx_xpm_file_to_image(MLX, "Texture/3.xpm", &TEXT_WIDTH[1],
			&TEXT_HEIGHT[1]);
	WALL_DATA[1] = mlx_get_data_addr(WALL[1], &TEXT_BPP[1], &TEXT_SIZE_LINE[1],
			&ENDIAN);
	WALL[2] = mlx_xpm_file_to_image(MLX, "Texture/end.xpm", &TEXT_WIDTH[2],
			&TEXT_HEIGHT[2]);
...
{
	WALL[7] = mlx_xpm_file_to_image(MLX, "Texture/1.xpm", &TEXT_WIDTH[7],
			&TEXT_HEIGHT[7]);
	WALL_DATA[7] = mlx_get_data_addr(WALL[7], &TEXT_BPP[7], &TEXT_SIZE_LINE[7],
			&ENDIAN);
}
