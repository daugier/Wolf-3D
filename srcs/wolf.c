...
/*   Updated: 2016/11/13 16:35:19 by daugier          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "wolf.h"

static void		display_touch(t_struct *data)
{
	int	color;
...
static void		display_start(t_struct *data)
{
	int	x;
	int	y;
	int	color;
...
static void		ft_moove(t_struct *data)
{
	if (MOOVE & WALK)
		ft_walk(data);
	if (MOOVE & BACK)
...
int				wolf(t_struct *data)
{
	...
	if (!START)
		...
		display_end(data, 10);
	else if (MAP[(int)POS_X][(int)POS_Y] == '2' && TIME > 0)
		display_end(data, 11);
	else if (TIME && START)
	{
		X = -1;
